...
BASELINE_RESULTS_PATH = PROJECT_ROOT / "results" / "baseline" / "golden_results_raw_972e97b8.json"
REFERENCE_CONTEXTS_PATH = PROJECT_ROOT / "evaluations" / "deepeval" / "reference_contexts.json"

JUDGE_MODEL_NAME = os.getenv("DEEPEVAL_JUDGE_MODEL", "qwen3:8b")
...
def apply_optional_case_filter(cases: list[dict]) -> list[dict]:
    ...
    if not filtered:
        raise ValueError(f"DEEPEVAL_CASE_IDS did not match any test cases: {sorted(allowed)}")
    return filtered

ALL_CASES = apply_optional_case_filter(build_atomic_cases())
FAITHFULNESS_CASES = [case for case in ALL_CASES if case["include_faithfulness"]]
JUDGE = create_judge()
...
def test_reference_grounded_faithfulness(case):
    ...
    assert_test(make_test_case(case, include_context=True), [metric], run_async=False)
